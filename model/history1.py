...
class SATA(Module):
    ...
    def ans(self,z,adj):
        ...
        return M
    def forward(self, feat, adj,value1=None,value2=None):
        ...
        for i in range(self.nlayer):
            ...
            #     print('SVDle')
            #     U, S, V =torch.svd(y_n)
            #     D = torch.eye(S.size(0)).cuda()*0.1
            #     S_modified = torch.diag(S) - D
            #     y_n = U @ S_modified @ V.t()
            #
            #     U, S, V = torch.svd(z_n)
            #     D = torch.eye(S.size(0)).cuda() * 0.1
            #     S_modified = torch.diag(S) - D
            #     z_n = U @ S_modified @ V.t()

            # if self.epoch%50==0:
            #     U, S, Vt = svd(z_n.cpu().detach().numpy(), full_matrices=False)
            #     D = np.eye(len(S))*0.01
            #     S_modified = np.diag(S) - D
            #     z_n =  U @ S_modified @ Vt
            #     z_n = torch.tensor(z_n, dtype=torch.float32).cuda()
            #
            #     U, S, Vt = svd(y_n.cpu().detach().numpy(), full_matrices=False)
            #     D = np.eye(len(S))*0.01
            #     S_modified = np.diag(S) - D
            #     y_n =  U @ S_modified @ Vt
            #     y_n = torch.tensor(y_n, dtype=torch.float32).cuda()
            y = y_n
            z = z_n
            EP = EP_n
            # if value1!= None and i!=0:
            #     EP= torch.where(abs(EP)>value1, EP, torch.tensor(0.0).cuda())
            #     self.w22= torch.where(abs(self.alpha*self.EA*self.w22)> value2, torch.tensor(1.0).cuda(), torch.tensor(0.0).cuda())
        self.epoch = self.epoch + 1
        y = F.normalize(y, p=2, dim=1)
        z = F.normalize(z, p=2, dim=1)
        p = torch.cat((z, y), dim=1)
        p = F.dropout(p, self.dropout, training=self.training)
        w23 = torch.where(self.alpha * self.EA > 0.5, self.w22, torch.tensor(0.0).cuda())
        return self.w2(p),EP,self.alpha*self.EA*self.w22,adj*EP+self.alpha*self.EA*w23
